    ...
    <div className="space-y-6 animate-fade-in-up">
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200">
        ...
        <form onSubmit={handleAddPrinter} className="flex flex-col md:flex-row gap-4 mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100">
          <input type="text" name="printerNameFunc" placeholder="Nome (Ex: Grelha / Bar)" value={printerForm.name} onChange={e => setPrinterForm({...printerForm, name: e.target.value})} className="border border-slate-300 p-3 rounded-xl flex-1 focus:outline-amber-500 font-bold text-slate-700" />
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200">
        ...
        <form onSubmit={handleAddGroup} className="flex flex-col md:flex-row gap-4 mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100">
          <input type="text" name="groupNameFunc" placeholder="Nome do Grupo (Ex: Bebidas)" value={groupForm.name} onChange={e => setGroupForm({...groupForm, name: e.target.value})} className="border border-slate-300 p-3 rounded-xl flex-1 focus:outline-amber-500 font-bold text-slate-700" />
          ...
          {productGroups.map(g => (
            <div key={g.id} className="flex justify-between items-center bg-white border border-slate-200 p-4 rounded-2xl shadow-sm">
              <div className="flex gap-6 items-center">
                ...
                {g.regraFiscalId && <span className="text-emerald-700 bg-emerald-100 px-3 py-1 rounded-lg text-xs font-bold">🧾 Regra Fiscal</span>}
              </div>
...
export default function FuncionariosPortal() {
  ...
  const allMenuItems = [
    ...
    { id: 'financeiro', label: 'Módulo Financeiro', icon: '🏦', isDropdown: true }, 
  ...
  // Extrair as permissões do usuário logado
  let currentPerms = [];
  if (employeeUser?.permissions && Array.isArray(employeeUser.permissions)) {
      currentPerms = employeeUser.permissions;
  } else if (employeeUser?.profile?.permissions) {
      try { currentPerms = JSON.parse(employeeUser.profile.permissions); } catch(e) { currentPerms = []; }
  }
  const hasPerm = (p) => currentPerms.includes('gestao') || currentPerms.includes(p);

  const permittedMenuItems = allMenuItems.filter(item => {
    if (hasPerm('gestao')) return true; 

    switch(item.id) {
      case 'pdv': return hasPerm('pdv'); 
      case 'salao': return hasPerm('salao') || hasPerm('pdv'); 
      case 'kds': return hasPerm('kds') || hasPerm('kanban') || hasPerm('kds_salao') || hasPerm('cozinha') || hasPerm('pdv');
      case 'expedicao': return hasPerm('expedicao') || hasPerm('entregas');
      case 'historico': return hasPerm('historico');
      case 'turnos': return hasPerm('turnos');
      case 'financeiro': return hasPerm('contas_pagar') || hasPerm('relatorio_financeiro'); 
      case 'produtos':
      case 'categorias':
      case 'promocoes': return hasPerm('produtos') || hasPerm('promocoes') || hasPerm('categorias');
      case 'estoque': return hasPerm('estoque');
      case 'analytics': return hasPerm('analytics');
      case 'crm': return hasPerm('crm');
      case 'fornecedores': return hasPerm('fornecedores');
      case 'impressoes': return hasPerm('config') || hasPerm('impressoes');
      case 'fiscal': return hasPerm('fiscal');
      case 'config': return hasPerm('config');
  ...
  const assignDelivery = async (orderIds, deliveryPersonId) => {
    try {
      ...
      if ((await res.json()).success) {
        ...
        logEmployeeAction(`Despachou pedido(s) para entrega`);
        fetchOrders();
...
const handleEditCategory = async (e) => {
  ...
  const handleDeleteCategory = async (categoryId) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { logEmployeeAction(`Excluiu uma categoria do cardápio`); fetchMenu(); } else alert(data.error);
  ...
  const handleAddCoupon = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { logEmployeeAction(`Criou novo cupom: ${couponForm.code}`); setCouponForm({ code: '', type: 'FIXED', value: '', minOrderValue: '0', maxUses: '0' }); fetchCoupons(); } 
  ...
  const handleConfirmXmlImport = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { logEmployeeAction(`Importou NFe XML (Chave: ${xmlPreviewData.chaveNfe})`); alert(data.message); setShowXmlModal(false); fetchInsumos(); fetchMovimentacoes(); fetchProducts(); } 
  ...
  const handleAddFicha = async (e, productId) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { logEmployeeAction(`Adicionou item à Ficha Técnica de um produto`); carregarFicha(productId); fetchProducts(); } else alert(data.error);
...
${dataBackend.error}`); }
  ...
  const handleSaveCnpj = async (cnpj) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { setFiscalData(newData); logEmployeeAction(`Alterou o CNPJ Emitente Fiscal`); alert('CNPJ Salvo!'); } else { alert('Erro ao salvar CNPJ.'); }
    } catch (error) { alert('Erro de conexão ao salvar CNPJ.'); }
  };

  const handleAddIcms = (e) => { e.preventDefault(); const updatedIcms = [...(fiscalData.icms || []), { ...formIcms, id: formIcms.id || Date.now().toString() }]; saveFiscalData({ ...fiscalData, icms: updatedIcms }); setFormIcms({ id: '', descricao: '', regime: 'Simples Nacional', cfop: '', cst: '', aliquota: '' }); logEmployeeAction('Adicionou regra ICMS'); };
  const handleDeleteIcms = (id) => { if(!confirm("Excluir Categoria ICMS?")) return; saveFiscalData({ ...fiscalData, icms: (fiscalData.icms || []).filter(x => x.id !== id) }); logEmployeeAction('Excluiu regra ICMS'); };
  const handleAddPis = (e) => { e.preventDefault(); const updatedPis = [...(fiscalData.pisCofins || []), { ...formPis, id: formPis.id || Date.now().toString() }]; saveFiscalData({ ...fiscalData, pisCofins: updatedPis }); setFormPis({ id: '', descricao: '', cstPis: '', aliqPis: '', cstCofins: '', aliqCofins: '' }); logEmployeeAction('Adicionou regra PIS/Cofins'); };
  const handleDeletePis = (id) => { if(!confirm("Excluir Categoria PIS/Cofins?")) return; saveFiscalData({ ...fiscalData, pisCofins: (fiscalData.pisCofins || []).filter(x => x.id !== id) }); logEmployeeAction('Excluiu regra PIS/Cofins'); };
  const handleAddIbsCbs = (e) => { e.preventDefault(); const updated = [...(fiscalData.ibsCbs || []), { ...formIbsCbs, id: formIbsCbs.id || Date.now().toString() }]; saveFiscalData({ ...fiscalData, ibsCbs: updated }); setFormIbsCbs({ id: '', descricao: '', cst: '000', classificacao: '000001', aliqIbsUf: '0.1', aliqCbs: '0.9' }); logEmployeeAction('Adicionou regra IBS/CBS'); };
  ...
  const handleEditCustomer = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { logEmployeeAction(`Editou cadastro de cliente: ${editingCustomer.name}`); alert('Cliente atualizado!'); setEditingCustomer(null); fetchCustomers(); } else alert('Erro.');
    } catch (error) { alert('Erro de conexão.'); }
  };
        ...
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] w-full max-w-md shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              ...
              <input type="password" required value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3.5 text-sm text-white focus:outline-none focus:border-amber-500" placeholder="••••••••" />
            </div>
            <button type="submit" disabled={loading} className="w-full bg-amber-500 hover:bg-amber-600 text-slate-950 font-black py-4 rounded-xl transition-all shadow-lg mt-2 cursor-pointer">
  ...
  if (loading) {
     return (
        <div className="flex h-screen items-center justify-center bg-slate-950 text-amber-500 font-bold">
           <div className="animate-pulse flex flex-col items-center">
              <div className="w-8 h-8 border-4 border-amber-500 border-t-transparent rounded-full animate-spin mb-4"></div>
              <p>Carregando Portal Operacional...</p>
           </div>
        </div>
     );
  }
    ...
    <div className="flex h-screen bg-slate-50 text-slate-800 font-sans overflow-hidden">
      ...
      <aside className={`relative flex flex-col bg-slate-950 border-r border-slate-900 transition-all duration-300 ease-in-out z-30 shadow-2xl ${isSidebarOpen ? 'w-72' : 'w-20'}`}>
        ...
        <nav className="flex-1 py-6 px-3 space-y-2 overflow-y-auto hide-scrollbar">
          <div className="mb-6 px-4 hidden md:block">
             <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Funcionário Logado:</p>
             <p className="text-white text-sm font-black truncate">{employeeUser?.name}</p>
             <p className="text-amber-500 text-xs font-bold truncate">{employeeUser?.role}</p>
          </div>
          {permittedMenuItems.map((item) => {
            
            // Renderiza o especial se for o menu em cascata
            if (item.isDropdown) {
              const isThisMenuOpen = item.id === 'kds' ? isKdsMenuOpen : isFinanceiroMenuOpen;
              ...
              return (
                <div key={item.id} className="flex flex-col">
                  <button 
                    onClick={toggleMenu} 
                    className={`w-full flex items-center gap-4 px-3 py-3.5 rounded-xl transition-all cursor-pointer ${isThisMenuOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
                  ...
                  >
                    ...
                    {isSidebarOpen && <span className="text-[10px] font-black">{isThisMenuOpen ? '▼' : '▶'}</span>}
                  </button>
                  
                  {isThisMenuOpen && isSidebarOpen && (
                    <div className="ml-4 mt-2 space-y-1 pl-4 border-l border-slate-200 animate-fade-in-up">
                      ...
                      {item.id === 'financeiro' && (
                        <>
                          {hasPerm('contas_pagar') && (
                            <button onClick={() => setActiveTab('financeiro/contas')} className="w-full flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-amber-500 hover:bg-white/5 py-2.5 px-3 rounded-lg transition-colors cursor-pointer text-left">
                              <span className="text-sm">💸</span> Contas a Pagar
                            </button>
                          )}
                          {hasPerm('relatorio_financeiro') && (
                            <button onClick={() => setActiveTab('financeiro/relatorio')} className="w-full flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-amber-500 hover:bg-white/5 py-2.5 px-3 rounded-lg transition-colors cursor-pointer text-left">
                              <span className="text-sm">📊</span> Relatório Detalhado
                            </button>
                          )}
                        </>
                      )}
            ...
            return (
              <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center gap-4 px-3 py-3.5 rounded-xl transition-all cursor-pointer ${isActive ? 'bg-amber-500 text-black shadow-md' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`} title={!isSidebarOpen ? item.label : ""}>
                <span className="text-xl shrink-0 flex items-center justify-center w-8">{item.icon}</span>
        ...
        <div className="p-4 border-t border-white/5">
          <button onClick={handleLogout} className={`w-full flex items-center gap-4 px-3 py-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-all cursor-pointer ${!isSidebarOpen && 'justify-center'}`} title={!isSidebarOpen ? "Desconectar" : ""}>
            ...
            {isSidebarOpen && <span className="font-bold whitespace-nowrap text-sm">Sair do Portal</span>}
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen relative overflow-hidden bg-slate-50">
        <header className="h-20 flex items-center justify-between px-8 bg-white border-b border-slate-200 shadow-sm z-20">
           <h1 className="text-2xl font-black text-slate-800">{permittedMenuItems.find(m => m.id === activeTab)?.label || (activeTab.includes('financeiro') ? 'Módulo Financeiro' : 'Acesso Restrito')}</h1>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 pb-32">
           ...
           {activeTab === 'financeiro/contas' && hasPerm('contas_pagar') && <ContasTab API_URL={API_URL} />}
           {activeTab === 'financeiro/relatorio' && hasPerm('relatorio_financeiro') && <RelatorioTab API_URL={API_URL} />}
        </div>
      </main>
