...
export default function DeliveryApp() {
  ...
  // 🚨 ATUALIZADO: Rota de confirmar entrega batendo no novo backend!
  const handleConfirmDelivery = async (order) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/delivery/confirm`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ shortId: order.shortId, code })
      });
      const data = await res.json();
     ...
     return (
        <div className="min-h-screen bg-[#0a0a0a] text-zinc-100 flex flex-col items-center justify-center p-4">
           <div className="w-full max-w-sm bg-[#121212] p-8 rounded-3xl border border-white/5 shadow-2xl text-center">
              ...
              {!capturedFace ? (
                 <button onClick={capturePhoto} className="w-full bg-amber-500 text-black font-black py-4 rounded-xl shadow-md active:scale-95">
                    📸 Tirar Foto
                 </button>
              ) : (
                 <div className="flex gap-2">
                    <button onClick={retakePhoto} className="flex-1 bg-white/10 text-white font-bold py-4 rounded-xl">Repetir</button>
                    <button onClick={authMode === 'register' ? () => setShowCamera(false) : handleFaceVerify} disabled={isProcessing} className="flex-1 bg-emerald-500 text-black font-black py-4 rounded-xl shadow-md">
        ...
        <div className="w-full max-w-sm bg-[#121212] p-8 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          {authMode === 'login' ? (
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <button type="submit" disabled={isProcessing} className={`w-full font-black text-lg py-4 rounded-2xl mt-4 transition-all shadow-md ${isProcessing ? 'bg-amber-500/50 text-black/50 cursor-not-allowed' : 'bg-amber-500 hover:bg-amber-400 text-black active:scale-95'}`}>
                {isProcessing ? 'Acessando...' : 'Entrar na Rota'}
              </button>
              <p className="text-center text-xs text-zinc-500 mt-4">
                 Novo por aqui? <button type="button" onClick={() => setAuthMode('register')} className="text-amber-500 font-bold underline">Cadastre-se</button>
              </p>
            </form>
          ) : (
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <button type="button" onClick={() => { setShowCamera(true); startCamera(); }} className={`w-full font-bold text-sm py-3 rounded-xl transition-all border ${capturedFace ? 'border-emerald-500 text-emerald-400 bg-emerald-500/10' : 'border-amber-500 text-amber-500 bg-amber-500/10 hover:bg-amber-500/20'}`}>
                 {capturedFace ? '✅ Rosto Registado (Alterar)' : '📸 Registar Rosto (Obrigatório)'}
              </button>

              <button type="submit" disabled={isProcessing} className="w-full font-black text-lg py-4 rounded-2xl mt-4 transition-all bg-amber-500 text-black active:scale-95">
                {isProcessing ? 'Aguarde...' : 'Criar Conta'}
              </button>
              <p className="text-center text-xs text-zinc-500 mt-4">
                 Já tem conta? <button type="button" onClick={() => setAuthMode('login')} className="text-amber-500 font-bold underline">Fazer Login</button>
              </p>
            </form>
          )}
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100 flex flex-col font-sans pb-10">
      <header className="bg-[#121212] border-b border-white/5 p-6 rounded-b-3xl shadow-2xl flex justify-between items-center z-10 sticky top-0 relative overflow-hidden">
        ...
        <button onClick={handleLogout} className="bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 p-3 rounded-xl font-bold shadow-md transition-colors text-sm">
           Sair
        </button>
      </header>
            ...
            <div key={order.id} className="bg-[#121212] p-5 rounded-3xl shadow-xl border border-white/5 relative overflow-hidden animate-fade-in-up">
              ...
              <div className="flex gap-2 pt-4 mt-4 border-t border-white/5 items-end ml-2">
                 ...
                 <button 
                    ...
                    className="bg-amber-500 hover:bg-amber-400 text-black font-black px-4 h-[60px] rounded-xl shadow-[0_0_15px_rgba(245,158,11,0.2)] active:scale-95 transition-all"
                 >
                    Confirmar
                 </button>
              </div>
