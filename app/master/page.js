...
export default function MasterDashboard() {
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      ...
      const payload = { 
        ...
        inscricaoEstadual: editingStore.inscricaoEstadual, inscricaoMunicipal: editingStore.inscricaoMunicipal,
        emailEmpresa: editingStore.emailEmpresa, telefoneEmpresa: editingStore.telefoneEmpresa,
  ...
  if (!isAuthenticated) {
    return (
      <div className="h-full flex items-center justify-center relative overflow-hidden bg-slate-50">
        <div className="bg-white border border-slate-200 p-8 rounded-[2rem] w-full max-w-sm shadow-2xl relative z-10 animate-fade-in-up">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-black text-[#0e4a56] tracking-tight">Zenix Master</h1>
          ...
          <form onSubmit={handleMasterLogin} className="space-y-5">
            <div>
               ...
               <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-900 focus:outline-none focus:border-[#f58220]" placeholder="admin@zenixfood.com" />
            </div>
            <div>
               <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex justify-between items-end mb-1">
                 <span>Senha</span>
                 <button type="button" onClick={() => { setIsForgotModalOpen(true); setForgotStatus('idle'); setForgotEmail(email); }} className="text-[#f58220] hover:text-[#e07318] normal-case">Esqueceu?</button>
               </label>
               <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-900 focus:outline-none focus:border-[#f58220]" placeholder="••••••••" />
            </div>
            ...
            <button type="submit" disabled={isLoginLoading} className="w-full bg-[#0e4a56] hover:bg-[#0a3842] text-white font-black py-4 rounded-xl transition-all shadow-lg mt-2 cursor-pointer flex items-center justify-center gap-2">
    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-slate-200 p-6 rounded-[2rem] shadow-sm">
          ...
          <p className="text-4xl font-black text-[#0e4a56]">{lojasAtivas} <span className="text-sm font-medium text-slate-400">/ {stores.length} total</span></p>
        </div>
        <div className="bg-[#f58220]/10 border border-[#f58220]/20 p-6 rounded-[2rem] shadow-sm">
          <p className="text-[10px] font-black text-[#f58220] uppercase tracking-widest mb-1">MRR Potencial</p>
          <p className="text-4xl font-black text-[#e07318]">R$ {stores.reduce((acc, store) => acc + Number(store.monthlyFee || 0), 0).toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <h2 className="font-black text-[#0e4a56] text-lg">Lojas e Inquilinos</h2>
          <button onClick={() => router.push('/master/stores/new')} className="bg-[#0e4a56] hover:bg-[#0a3842] text-white px-5 py-2.5 rounded-xl font-black shadow-md cursor-pointer text-xs">
            + Cadastrar Nova Loja
          </button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-white border-b border-slate-200 text-slate-400 uppercase tracking-widest text-[10px] font-black">
              <tr>
                ...
                return (
                  <tr key={store.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-5">
                       ...
                       {store.adminUser ? (
                         <span className="bg-sky-50 border border-sky-100 text-sky-600 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest flex items-center w-fit gap-1 mt-1">👥 Franqueado: {store.adminUser.name}</span>
                       ) : (
                    ...
                    <td className="px-6 py-5">
                       <span className="bg-purple-50 text-purple-600 px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-wider">
          ...
          <div className="bg-white border border-slate-200 p-6 md:p-10 rounded-[2.5rem] w-full max-w-4xl shadow-2xl relative flex flex-col max-h-[92vh] animate-fade-in-up">
            ...
            <form onSubmit={handleSaveEdit} className="overflow-y-auto pr-2 space-y-6 flex-1 hide-scrollbar">
              ...
              <div className="bg-sky-50 p-6 rounded-3xl border border-sky-100 space-y-4">
                <h3 className="text-xs font-black text-sky-600 uppercase tracking-widest flex items-center gap-2"><span>👔</span> Responsável & Acesso Master da Loja</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><label className="text-[10px] font-black text-slate-500 uppercase block mb-1">Nome Completo do Dono</label><input type="text" required value={editingStore.nomeResponsavel} onChange={e => setEditingStore({...editingStore, nomeResponsavel: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3.5 text-sm text-slate-900 font-bold shadow-sm focus:border-sky-500 outline-none" /></div>
                  <div><label className="text-[10px] font-black text-slate-500 uppercase block mb-1">CPF</label><input type="text" required value={editingStore.cpfResponsavel} onChange={handleEditCpfChange} className="w-full bg-white border border-slate-300 rounded-xl p-3.5 text-sm text-slate-900 font-mono shadow-sm focus:border-sky-500 outline-none" /></div>
                  <div><label className="text-[10px] font-black text-sky-700 uppercase block mb-1">E-mail de Acesso (Login)</label><input type="email" required value={editingStore.emailResponsavel} onChange={e => setEditingStore({...editingStore, emailResponsavel: e.target.value})} className="w-full bg-white border border-sky-300 rounded-xl p-3.5 text-sm text-slate-900 font-bold shadow-sm focus:border-sky-500 outline-none" /></div>
                  <div><label className="text-[10px] font-black text-sky-700 uppercase block mb-1">Nova Senha Master (Opcional)</label><input type="text" value={editingStore.senhaResponsavel} onChange={e => setEditingStore({...editingStore, senhaResponsavel: e.target.value})} placeholder="Deixe em branco para manter a atual" className="w-full bg-white border border-sky-300 rounded-xl p-3.5 text-sm text-slate-900 shadow-sm focus:border-sky-500 outline-none" /></div>
                </div>
              </div>
              ...
              <div className="bg-purple-50 p-6 rounded-3xl border border-purple-100 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full bg-white border border-purple-300 rounded-xl p-3.5 text-sm text-slate-900 font-bold shadow-sm cursor-pointer focus:border-purple-500 outline-none"
                    >
                ...
                <div className="bg-white p-4 rounded-2xl border border-purple-200 mt-2 flex flex-col gap-3">
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <span className="text-sm font-bold text-slate-800">⌨️ Adicionar Suporte Técnico Extra</span>
                  </label>
                  {editingStore.temSuporte && (
                    <div className="animate-fade-in-up mt-2">
                      ...
                      <input 
                        ...
                        className="w-full md:w-1/2 bg-white border border-purple-300 rounded-xl p-3 text-sm text-purple-800 font-black shadow-sm focus:border-purple-500 outline-none" 
                      />
                    </div>
                  )}
                </div>
              ...
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 space-y-4">
                <h3 className="text-xs font-black text-[#0e4a56] uppercase tracking-widest flex items-center gap-2"><span>📍</span> Endereço Completo</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div><label className="text-[10px] font-black text-slate-500 uppercase block mb-1">CEP</label><input type="text" value={editingStore.cep} onChange={handleEditCepSearch} placeholder="00000-000" className="w-full bg-white border border-slate-300 rounded-xl p-3.5 text-sm text-slate-900 font-mono shadow-sm focus:border-[#0e4a56] outline-none" /></div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div><label className="text-[10px] font-black text-slate-500 uppercase block mb-1">Número</label><input type="text" value={editingStore.number} onChange={e => setEditingStore({...editingStore, number: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3.5 text-sm text-slate-900 shadow-sm focus:border-[#0e4a56] outline-none" /></div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div><label className="text-[10px] font-black text-slate-500 uppercase block mb-1">Cidade</label><input type="text" value={editingStore.city} onChange={e => setEditingStore({...editingStore, city: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3.5 text-sm text-slate-900 shadow-sm focus:border-[#0e4a56] outline-none" /></div>
                    <div><label className="text-[10px] font-black text-slate-500 uppercase block mb-1">UF</label><input type="text" maxLength={2} value={editingStore.state} onChange={e => setEditingStore({...editingStore, state: e.target.value.toUpperCase()})} className="w-full bg-white border border-slate-300 rounded-xl p-3.5 text-sm text-slate-900 uppercase font-mono text-center shadow-sm focus:border-[#0e4a56] outline-none" /></div>
                  </div>
                </div>
              </div>

              <div className="pt-4 flex gap-4 shrink-0 mt-4 border-t border-slate-100">
                <button type="button" onClick={() => setEditingStore(null)} className="flex-1 bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 font-bold py-4 rounded-2xl cursor-pointer text-sm transition-colors">Cancelar</button>
                <button type="submit" className="flex-2 bg-[#0e4a56] hover:bg-[#0a3842] text-white font-black py-4 rounded-2xl shadow-md cursor-pointer active:scale-95 text-base transition-colors">Salvar Alterações</button>
              </div>
      ...
      {viewingInvoicesStore && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 p-8 rounded-[2rem] w-full max-w-3xl shadow-2xl relative flex flex-col max-h-[90vh] animate-fade-in-up">
            <div className="flex justify-between items-center mb-6 shrink-0 border-b border-slate-100 pb-4">
              <div>
                <h2 className="text-2xl font-black text-[#0e4a56] flex items-center gap-3"><span className="text-emerald-500">💳</span> Central de Cobranças</h2>
              ...
              <button onClick={() => setViewingInvoicesStore(null)} className="w-10 h-10 bg-slate-100 hover:bg-red-100 hover:text-red-500 text-slate-500 rounded-full flex items-center justify-center font-black text-lg cursor-pointer transition-colors">✕</button>
            </div>
