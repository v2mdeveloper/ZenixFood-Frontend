...
export default function RecepcaoHostessPage() {
  ...
  const openCreateModal = () => {
    setIsEditing(false); setNovoEventoForm({ id: null, nome: '', tipo: 'MISTO', dataHoraInicio: toLocalISOString(new Date()), dataHoraFim: '', qtdPessoas: 1, observacoes: '' }); setIsCreateModalOpen(true);
  };
  ...
  const handleFinalizarEvento = async (eventoId) => {
    if(!confirm("Tem certeza que deseja finalizar este evento? O relatório financeiro será gerado e guardado.")) return;
    try {
      const res = await fetchWithStore(`${API_URL}/api/eventos/${eventoId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'FINALIZADO' }) });
      const data = await res.json();
      if (res.ok && data.success) { alert("Evento finalizado com sucesso!"); setSelectedEvento(null); fetchEventos(); } else alert(data.error || "Erro ao tentar finalizar.");
  ...
  const handleDownloadRelatorio = async (evento) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        const formatMoney = (val) => String(val).replace('.', ','); 
        let csvContent = "NOME;CPF;EMAIL;MESA;COMANDA;CHECK-IN;GASTO INDIVIDUAL (R$)\n";
        data.convidados.forEach(c => { csvContent += `"${c.nome}";"${c.cpf}";"${c.email}";"${c.mesa}";"${c.comanda}";"${c.checkIn}";"R$ ${formatMoney(c.gasto)}"\n`; });
        ...
        if (data.mesasColetivas && data.mesasColetivas.length > 0) {
           ...
           data.mesasColetivas.forEach(m => { csvContent += `"Mesa ${m.mesa}";"${m.qtdItens} itens";"R$ ${formatMoney(m.gasto)}"\n`; });
        }
        csvContent += `\n;;;;;TOTAL GERAL DO EVENTO;"R$ ${formatMoney(data.totalGasto)}"\n`;

        const blob = new Blob(["\ufeff" + csvContent], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `Relatorio_${data.eventoNome.replace(/\s+/g, '_')}.csv`; document.body.appendChild(link); link.click(); document.body.removeChild(link);
  ...
  const handleDownloadTemplate = () => {
    ...
    const blob = new Blob(["\ufeff" + csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'Zenix_Modelo_Convidados.csv'; document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const headers = lines[0].split(',').map(h => h.trim().toLowerCase()); const convidadosArray = [];
      for (let i = 1; i < lines.length; i++) {
        const data = lines[i].split(',');
        if (data.length >= headers.length) {
          let rowObj = {};
          headers.forEach((header, index) => {
            let cleanHeader = header.replace(/["\r]/g, ''); let cleanData = data[index] ? data[index].replace(/["\r]/g, '').trim() : '';
  ...
  const confirmImport = async () => {
    if (csvPreview.length === 0) return alert('Nenhum convidado para importar.'); setIsImporting(true);
    try {
      const res = await fetchWithStore(`${API_URL}/api/eventos/${selectedEvento.id}/importar`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ convidados: csvPreview }) });
      const data = await res.json();
      if (res.ok) { alert(data.message); setShowImportModal(false); setCsvPreview([]); fetchEventos(); setSelectedEvento(null); } else alert(data.error);
  ...
  const changeMonth = (offset) => { const newDate = new Date(currentDate); newDate.setMonth(currentDate.getMonth() + offset); setCurrentDate(newDate); };
  ...
  const renderCalendar = () => {
    ...
    return (
      <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
         <div className="flex justify-between items-center mb-4"><button onClick={() => changeMonth(-1)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 font-black text-slate-500">{"<"}</button><h3 className="font-black text-lg text-slate-800 uppercase tracking-widest">{currentDate.toLocaleString('pt-BR', { month: 'long', year: 'numeric' })}</h3><button onClick={() => changeMonth(1)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 font-black text-slate-500">{">"}</button></div>
         <div className="grid grid-cols-7 gap-2 mb-2 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">{weekDays.map(wd => <span key={wd}>{wd}</span>)}</div>
  ...
  const mesasDoEvento = extratoSeguro.mesasColetivas || [];
  const totalEventoGasto = Number(extratoSeguro.totalGasto) || 0;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col md:h-screen relative">
      ...
      <header className="bg-slate-900 text-white p-4 flex flex-col md:flex-row justify-between items-center shadow-lg shrink-0 gap-4 no-print">
         <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-start">
            <div className="flex items-center gap-3"><span className="text-3xl hidden sm:block">🥂</span><div><h1 className="text-lg md:text-xl font-black leading-none">Recepção</h1><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Hostess: {employeeUser?.name}</p></div></div>
        ...
        <div className={`w-full md:w-1/3 flex flex-col gap-4 ${selectedEvento ? 'hidden md:flex' : 'flex'}`}>
           {activeTab === 'HOJE' && (
             <div className="flex flex-col h-full gap-4">
                <div className="bg-white border border-slate-200 p-5 rounded-3xl shadow-sm flex justify-between items-center shrink-0"><h2 className="font-black text-lg">Eventos de Hoje</h2><button onClick={openCreateModal} className="bg-blue-600 hover:bg-blue-700 text-white w-10 h-10 rounded-full font-black text-xl flex items-center justify-center shadow-md">+</button></div>
           ...
           {activeTab === 'CALENDARIO' && (
             <div className="flex flex-col h-full gap-4">
                {renderCalendar()}
                <div className="bg-white border border-slate-200 p-5 rounded-3xl shadow-sm flex justify-between items-center shrink-0"><h2 className="font-black text-lg text-blue-700">Agenda: {selectedDate.toLocaleDateString('pt-BR')}</h2><button onClick={openCreateModal} className="bg-blue-600 hover:bg-blue-700 text-white w-10 h-10 rounded-full font-black text-xl flex items-center justify-center shadow-md">+</button></div>
                <div className="flex-1 overflow-y-auto space-y-3 hide-scrollbar pb-10">
           ...
           {activeTab === 'HISTORICO' && (
             <div className="flex flex-col h-full gap-4">
                <div className="bg-white border border-slate-200 p-5 rounded-3xl shadow-sm flex flex-col shrink-0"><h2 className="font-black text-lg text-emerald-700">Histórico de Eventos</h2><p className="text-xs text-slate-500 font-medium mt-1">Eventos já finalizados.</p></div>
                <div className="flex-1 overflow-y-auto space-y-3 hide-scrollbar pb-10">
             ...
             <>
               <div className="p-4 md:p-6 border-b border-slate-100 bg-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
                  ...
                  {activeTab !== 'HISTORICO' ? (
                     <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto mt-4 md:mt-0">
                        ...
                        <button onClick={() => setShowImportModal(true)} className="w-full md:w-auto bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3 rounded-xl font-black shadow-md flex items-center justify-center gap-2 cursor-pointer transition-transform active:scale-95">📥 Importar (CSV)</button>
                     </div>
                  ) : (
                     <button onClick={() => handleDownloadRelatorio(selectedEvento)} className="w-full md:w-auto mt-4 md:mt-0 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl font-black shadow-md flex items-center justify-center gap-2 cursor-pointer transition-transform active:scale-95">📊 Baixar Relatório (Excel)</button>
                  )}
               </div>
               ...
               {mesasDoEvento.length > 0 && (
                  <div className="p-4 bg-blue-50 border-b border-blue-100 flex flex-wrap gap-2 shrink-0">
                     ...
                     {mesasDoEvento.map(m => {
                        return (
                          <button key={m.mesa} onClick={() => setViewingTable(m)} className="bg-white border border-blue-200 px-3 py-2 rounded-xl flex gap-3 items-center shadow-sm hover:border-blue-400 cursor-pointer transition-colors active:scale-95">
                             <span className="font-bold text-xs text-blue-800">Mesa {m.mesa}</span>
                             <span className="font-black text-xs text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">R$ {Number(m.gastoBruto || 0).toFixed(2)}</span>
                           ...
                           <div key={conv.id} onClick={() => setViewingGuest(conv)} className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-blue-300 transition-colors cursor-pointer group">
                              <div className="flex-1">
                                 <p className="font-black text-slate-800 text-base flex items-center gap-2">
                                    {conv.nome}
                                    <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest shadow-sm">💰 R$ {Number(conv.gastoBruto || 0).toFixed(2)}</span>
                                 </p>
                                 <div className="flex flex-wrap gap-2 mt-2">
                                    {conv.mesaIndicada && conv.mesaIndicada !== '-' && <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest">🪑 Mesa {conv.mesaIndicada} {conv.posicaoMesa && `- ${conv.posicaoMesa}`}</span>}
                                    {conv.comandaIndicada && conv.comandaIndicada !== '-' && <span className="bg-purple-100 text-purple-700 px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest">💳 Cmd {conv.comandaIndicada}</span>}
                                 </div>
                              </div>
                              <div className="w-full md:w-auto text-right flex items-center justify-end gap-3">
                                 {conv.checkIn === 'SIM' || conv.statusCheckIn ? (
                                    <div className="flex flex-col items-end">
                                       <span className="text-xs font-black text-emerald-500 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200">✅ Check-in Concluído</span>
                                       {conv.statusConta && (
                                          <span className={`mt-2 text-[9px] font-black px-2 py-1 rounded uppercase tracking-widest shadow-sm ${conv.statusConta === 'OPEN' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>
                                             {conv.statusConta === 'OPEN' ? '🟢 CONTA ABERTA' : '✅ CONTA PAGA/FECHADA'}
                                          </span>
                                       )}
                                    </div>
                                 ) : (
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-4 max-h-60 overflow-y-auto hide-scrollbar">
                 ...
                 {viewingTable.itemsSnapshot?.length > 0 ? (
                    <div className="space-y-2">
                       {viewingTable.itemsSnapshot.map(item => (
                          <div key={item.id} className="flex justify-between text-sm border-b border-slate-100 pb-2">
                             ...
                             <span className="font-black text-slate-900">R$ {(Number(item.price) * item.quantity).toFixed(2)}</span>
                          </div>
                       ))}
                    </div>
                 ) : (
           ...
           <div className="bg-white p-6 md:p-8 rounded-3xl w-full max-w-md shadow-2xl relative animate-fade-in-up print-area">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-4 max-h-60 overflow-y-auto hide-scrollbar">
                 ...
                 {viewingGuest.itemsSnapshot?.length > 0 ? (
                    <div className="space-y-2">
                       {viewingGuest.itemsSnapshot.map(item => (
                          <div key={item.id} className="flex justify-between text-sm border-b border-slate-100 pb-2">
                             <span className="font-bold text-slate-700">{item.quantity}x {item.name}</span>
                             <span className="font-black text-slate-900">R$ {(Number(item.price) * item.quantity).toFixed(2)}</span>
                          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 no-print">
           ...
           <div className="bg-white p-6 md:p-8 rounded-3xl w-full max-w-2xl shadow-2xl animate-fade-in-up flex flex-col max-h-[90vh]">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 shrink-0 border-b pb-4 gap-4">
                 ...
                 <div className="flex gap-2 w-full md:w-auto"><button onClick={handleDownloadTemplate} className="flex-1 md:flex-none bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded-xl text-xs font-black transition-colors">Baixar Modelo</button><button onClick={() => { setShowImportModal(false); setCsvPreview([]); }} className="w-10 h-10 rounded-full bg-slate-100 text-slate-500 font-black hover:text-red-500 transition-colors shrink-0">✕</button></div>
              </div>
              {csvPreview.length === 0 ? (
                 <div className="flex-1 border-2 border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center p-6 md:p-10 bg-slate-50 relative group hover:border-blue-500 transition-colors">
                    <span className="text-5xl mb-4 group-hover:scale-110 transition-transform">📄</span><p className="font-black text-slate-700 mb-2 text-center">Clique para procurar o arquivo .CSV</p><input type="file" accept=".csv" onChange={handleFileUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                 </div>
              ) : (
                 <div className="flex-1 overflow-y-auto border border-slate-200 rounded-2xl p-4 hide-scrollbar">
                    ...
                    <div className="space-y-2">
                       {csvPreview.slice(0, 50).map((conv, idx) => (
                          <div key={idx} className="bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs flex justify-between"><div><span className="font-black text-slate-800 block">{conv.nome}</span> <span className="text-slate-500">{conv.cpf || conv.email}</span></div><div className="text-right">{conv.mesaIndicada && <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded ml-1 font-bold">Mesa {conv.mesaIndicada}</span>}{conv.comandaIndicada && <span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded ml-1 font-bold">Cmd {conv.comandaIndicada}</span>}</div></div>
                       ))}
                    </div>
                 </div>
              )}
