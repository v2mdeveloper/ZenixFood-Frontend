...
function FuncionariosPortal({ storeSlug }) {
  ...
  const permittedMenuItems = allMenuItems.filter(item => {
    ...
    if (employeeUser?.permissions && Array.isArray(employeeUser.permissions)) {
       perms = employeeUser.permissions;
    } else if (employeeUser?.profile?.permissions) {
       try { perms = JSON.parse(employeeUser.profile.permissions); } catch(e) { perms = []; }
    }
              ...
              return (
                <div key={item.id} className="flex flex-col">
                  <button 
                    ...
                    className={`w-full flex items-center gap-4 px-3 py-3.5 rounded-xl transition-all cursor-pointer ${isKdsMenuOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
